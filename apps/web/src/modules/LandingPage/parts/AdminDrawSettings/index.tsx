...
function AdminDrawSetting() {
  ...
  const { startNewDraw, isPendingStartNewDraw } = useStartNewDraw(amountInWei);
  const { uploadPrize, isPendingUploadPrize } = useUploadPrize(amountInWei);
  const { setDrawTime, isPendingSetDrawTime } = useSetDrawTime();
  const { performDraw, isPendingPerformDraw } = usePerformDraw();
  ...
  return (
    <div className="flex flex-col w-full p-4 lg:p-8 rounded-lg bg-[#0B2D4680] backdrop-blur-[50px]">
      ...
      <div className="py-5">
        <Button
          loading={isPendingStartNewDraw}
          disabled={!canStartNewDraw}
